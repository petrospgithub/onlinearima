      ...
      if (!spline.isEmpty && spline.length >= h) {
        ...
        while (predictions<=Horizon) {
          ...
          val point=STPoint(
            ...
            OARIMA_ons.prediction(data_lon, state_new.w_lon.get, lonMeandiff),
            OARIMA_ons.prediction(data_lat,  state_new.w_lat.get, latMeandiff),

            OARIMA_ons.prediction(data_speed, state_new.w_speed.get, speedMeandiff),
            OARIMA_ons.prediction(data_heading,  state_new.w_heading.get, headingMeandiff),
            error = false
          )

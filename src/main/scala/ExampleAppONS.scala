...
object ExampleAppONS extends App{
  ...
  while (j<window) {
    w(j)=ThreadLocalRandom.current().nextInt(minimum, maximum+1)/100.0
    eye(j)=epsilon
    j=j+1
  }
  ...
  //todo for loop testing and migrate to spark
...
*/
  ...
  while (j<lon_arr.size-window) {
    ...
    val ret_ogd_lon=OARIMA_ons.call(reg_lon._1.slice(0,reg_lon._1.length-1), opt, reg_lon._1.last, i)
    val ret_ogd_lat=OARIMA_ons.call(reg_lat._1.slice(0,reg_lat._1.length-1), opt, reg_lat._1.last, i)
    ...
    buf_lon.appendAll(reg_lon._1.slice(2,reg_lon._1.length))
    buf_lat.appendAll(reg_lat._1.slice(2,reg_lat._1.length))
...
*/
    ...
    //todo mean/std online!
    val diff_lon=Normalize.gaussian_normalization(lon_arr.slice(j,j+window+1).toArray)
    val diff_lat=Normalize.gaussian_normalization(lat_arr.slice(j,j+window+1).toArray)

    //println("training + real(last one): "+diff_lon._1.toList)
    //println("training: "+diff_lon._1.slice(0,diff_lon._1.length-1).toList)

    val ret_ogd_lon=OARIMA_ons.call(diff_lon._1.slice(0,diff_lon._1.length-1), opt, diff_lon._1.last, i)
    val ret_ogd_lat=OARIMA_ons.call(diff_lat._1.slice(0,diff_lat._1.length-1), opt, diff_lat._1.last, i)
    ...
    /*
    buf_lon.appendAll(diff_lon._1.slice(1,diff_lon._1.length-1))
    buf_lat.appendAll(diff_lat._1.slice(1,diff_lat._1.length-1))
...
*/
    ...
    /*
        ...
        val ret_ogd_lon=OARIMA_ons.call(diff_lon.slice(0,diff_lon.length-1), opt, diff_lon.last, i)
        val ret_ogd_lat=OARIMA_ons.call(diff_lat.slice(0,diff_lon.length-1), opt, diff_lat.last, i)
    ...
    /*arma*/
    /*
    val ret_ogd_lon=OARIMA_ons.call(lon_arr.slice(j,j+window).toArray, opt, lon_arr(window), i)
    val ret_ogd_lat=OARIMA_ons.call(lat_arr.slice(j,j+window).toArray, opt, lat_arr(window), i)

...
object OnlineArimaGradientDescent {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val mappingFunc = (key: Integer, input: Option[AvroPoint], state: State[OARIMAstate]) => {

      val new_point: AvroPoint = input.get
      val h: Int = broadcastVar.value("historical_positions")

      val curr_state: OARIMAstate = if (state.exists()) {
        ...
        if (new_point.getTimestamp - points_state.state.last.getTimestamp > threshold_broadcastVar.value("GAP_PERIOD")) { //TODO gap threshold
          ...
          val oarima:OARIMAstate=state.get()

          //todo edw na allazw to w!!!
          val w_lon=OARIMA_ogd.adapt_w(oarima.prediction.getLongitude, new_point.getLongitude,oarima.w_lon,lrateVar.value, oarima.lon, oarima.i)
      ...
      val spline: Array[AvroPoint] = mode match {
        case false => Copy.deepCopy(curr_state.state.sortWith(_.getTimestamp < _.getTimestamp).toArray).toArray
        case true => {
